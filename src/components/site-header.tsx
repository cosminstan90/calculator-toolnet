      ...
      <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-4 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/calculatoare"
              className="flex min-h-11 items-center rounded-full bg-emerald-300 px-4 py-2 text-sm font-semibold text-slate-950 transition-transform duration-200 hover:-translate-y-0.5"
            >
              Exploreaza
            </Link>
          </div>
        ...
        <div className="relative xl:hidden">
          <nav className="-mx-1 no-scrollbar flex gap-2 overflow-x-auto pb-4">
            {primaryLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="flex min-h-11 shrink-0 items-center rounded-full border border-white/10 bg-white/6 px-3 py-2 text-xs font-semibold text-slate-200"
              >
                {link.label}
              </Link>
            ))}
          ...
          <span
            aria-hidden="true"
            className="pointer-events-none absolute top-0 bottom-4 right-0 w-10 bg-gradient-to-l from-slate-950 to-transparent"
          />
        </div>
      </div>
    </header>
